...
/* ------------------------------------------------------------------ *
 * Comptes multi-appareils plafonnés (ex. compte démo).
 *
 * Un compte qui porte un entier positif `deviceLimit` bascule sur un
 * modèle "registre plafonné" : `account.devices` devient une map
 * indexée par deviceId (tous types confondus), et non plus les deux
 * slots nommés desktop/mobile. Les comptes sans `deviceLimit` gardent
 * intégralement le comportement historique à deux slots.
 * ------------------------------------------------------------------ */

export function deviceLimitFor(account) {
  const n = account ? Number(account.deviceLimit) : 0;
  return Number.isInteger(n) && n > 0 ? n : 0;
}

export function ensureDeviceMap(account) {
  return account && account.devices && typeof account.devices === 'object'
    ? account.devices
    : {};
}

export function countDevices(deviceMap) {
  return deviceMap && typeof deviceMap === 'object'
